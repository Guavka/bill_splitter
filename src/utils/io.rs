...
/// Запрашивает у пользователя ввод положительного числа, больше или равного заданному минимальному значению.
///
/// Эта функция выводит сообщение `msg` и ожидает ввода от пользователя.
/// Если происходит ошибка при чтении или парсинге, выводится сообщение `error_msg`.
/// Если введенное значение меньше `min_value`, выводится сообщение `positive_msg`.
/// Функция продолжает запрашивать ввод до тех пор, пока не будет получено корректное значение.
///
/// # Параметры
///
/// - `msg`: Сообщение, которое будет выведено пользователю перед запросом ввода.
/// - `min_value`: Минимально допустимое значение для ввода.
/// - `error_msg`: Сообщение, которое будет выведено в случае ошибки при чтении ввода или парсинга.
/// - `positive_msg`: Сообщение, которое будет выведено, если введенное значение меньше `min_value`.
///
/// # Возвращаемое значение
///
/// Возвращает значение типа `T`, введенное пользователем, которое больше или равно `min_value`.
///
/// # Примечания
///
/// Функция будет продолжать запрашивать ввод до тех пор, пока не будет получено
/// корректное значение, которое больше или равно `min_value`.
pub fn get_number_positive<T>(msg: &str, min_value: T, error_msg: &str, positive_msg: &str) -> T
where
